...
describe('directoryTree', () => {
	...
	it('should execute a callback function for each file with no specified extensions', () => {
		...
		const tree = dirtree('./test/test_data', null, function(item) {
	...
	it('should execute a callback function for each directory', () => {
		...
		const tree = dirtree('./test/test_data', null, null, function(item) {
			callback_executed_times++;
		});
	...
	it('should execute a callback function for each file with specified extensions', () => {
		...
		const tree = dirtree('./test/test_data', { extensions: /\.txt$/ }, function(item) {
			callback_executed_times++;
		});
		expect(callback_executed_times).to.equal(number_of_files);
	});
	...
	it('should display the size of a directory (summing up the children)', () => {
		const tree = dirtree('./test/test_data', { attributes: ['size'], dirContentsSize: true, extensions: /\.txt$/ });
	...
	it('should return the correct exact result', () => {
		const tree = dirtree('./test/test_data', { attributes: ['size'], dirContentsSize: true, normalizePath: true });
	...
	it('should exclude the correct folders', () => {
		const tree = dirtree('./test/test_data', { attributes: ['size'], dirContentsSize: true, exclude: /another_dir/, normalizePath: true });
		expect(tree).to.deep.equal(excludeTree);
	});

	it('should exclude multiple folders', () => {
		const tree = dirtree('./test/test_data', { attributes: ['size'], dirContentsSize: true, exclude: [/another_dir/, /some_dir_2/], normalizePath: true });
